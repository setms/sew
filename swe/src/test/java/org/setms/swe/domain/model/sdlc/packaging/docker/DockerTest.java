...
class DockerTest {
  ...
  private void assertThatDockerComposeHasAppBuildInstruction(Resource<?> resource) {
    ...
    assertThat(resource.readAsString())
        .as(
            "docker-compose.yml should include build instruction under app profile, port 8080, active 'local' Spring profile, and no depends_on without a database")
        .contains("build: .")
        .contains("app")
        .contains("8080:8080")
  ...
  private void assertThatDockerComposeHasPostgreSqlContainer(Resource<?> resource) {
    ...
    assertThat(resource.readAsString())
        .as(
            "docker-compose.yml should include a 'db' service with postgres image, POSTGRES_PASSWORD, POSTGRES_DB, port 5432, health check, app depends_on, and SPRING_DATASOURCE_URL")
        .contains("db:")
        ...
        .contains("pg_isready")
        .contains("depends_on:")
        .contains("SPRING_DATASOURCE_URL: jdbc:postgresql://db:5432/" + PROJECT_NAME);
  }
}
