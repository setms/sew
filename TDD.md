...
- [x] Service implementation (2nd artifact):
  - `actual.getName()` equals `"ProjectsServiceImpl"`
  - `actual.getPackage()` equals `"com.company.project.domain.services"`
...
- [~] Warn about missing domain service when aggregate has an acceptance test with an aggregate
...
- [ ] No warning when domain service code already exists
- [ ] Applying the suggestion creates code artifacts generated by the `CodeGenerator`

### `JavaUnitTestGeneratorTest` — update service field to use implementation class

- [ ] Generated unit test field for aggregate scenario uses implementation class:
...
- [ ] Same for policy scenario: `new SendNotificationServiceImpl()`
