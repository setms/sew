...
- [x] When Gradle is initialized and test sources have a compilation error, `build()` emits a diagnostic with the compiler's error message and the line number in the test file where the error occurred.
- [x] `CodeTool` calls `buildTool.build()` after confirming the build tool is initialized.
